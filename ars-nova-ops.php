...
add_action( 'rest_api_init', function () {
	...
	// Install (or update, if overwrite=true). Source: slug | url | zip_b64.
...
function ans_ops_route_install( WP_REST_Request $req ) {
	...
	$sources = array_filter( array( 'slug' => $slug, 'url' => $url, 'zip_b64' => ( '' !== $zip_b64 ) ? '1' : '' ) );
	if ( 1 !== count( $sources ) ) {
		return new WP_Error( 'ans_ops_bad_source', 'Provide exactly one of: slug, url, zip_b64.', array( 'status' => 400 ) );
	}
